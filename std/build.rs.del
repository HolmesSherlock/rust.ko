...
const CLANG_ARGS_BLACKLIST: [&'static str; 11] = [
	...
	"-fno-delete-null-pointer-checks", "-no-pie"
];
...
fn main() {
	...
	let mut clang_args = match std::env::var("STD_CLANG_ARGS") {
		Ok(string) =>
	...
	let mut kernel_path = match std::env::var("STD_KERNEL_PATH") {
		Ok(string) => string,
		Err(error) => {
	...
	let mut filepath_header = format!("{}/{}", out_dir, FILENAME_HEADER);
	//let filepath_header = String::from("/usr/src/linux-headers-4.2.0-41-generic/include/linux/printk.h");
	...
	
	{
		...
		//let logger = Logger {};
		match bindgen::Bindings::generate(options, None) {
			Ok(result) => {
		...
		/*let bindings = bindgen::Builder::default()
			//.clang_arg("-I/usr/src/linux-headers-4.2.0-41-generic/include/")
            .clang_arg(clang_args.iter())
	        // The input header we would like to generate
	        // bindings for.
	        .header(filepath_header)
	    ...
	    let out_path = PathBuf::from("src/os/kernel.rs");
	    bindings
